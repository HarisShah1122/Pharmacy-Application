...
const CliniciansTable = () => {
  ...
  useEffect(() => {
    ...
    const url = listId
      ? `http://localhost:8081/clinicians?clinician_list_id=${listId}`
      : 'http://localhost:8081/clinicians';
  ...
  const handleSubmit = (e) => {
    ...
    fetch('http://localhost:8081/clinicians', {
      method: 'POST',
                              ...
                              <CDropdownItem
                                onClick={() => {
                                  if (window.confirm('Are you sure you want to delete this clinician?')) {
                                    fetch(`http://localhost:8081/clinicians/${item.id}`, {
                                      method: 'DELETE',
                                    })
                                      .then((res) => {
                                        if (!res.ok) throw new Error('Failed to delete clinician');
                                        setClinicians(clinicians.filter((clin) => clin.id !== item.id));
                                      })
                                      .catch((error) => {
