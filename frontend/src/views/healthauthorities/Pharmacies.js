...
const Pharmacies = () => {
  ...
  const fetchPharmacies = async () => {
    ...
    setError('');
    try {
      const response = await fetch('http://localhost:8081/pharmacies');
      if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = mode === 'add' ? 'http://localhost:8081/register' : `http://localhost:8081/pharmacies/${selectedPharmacyId}`;
  ...
  const handleDelete = async (pharmacyId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/pharmacies/${pharmacyId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
