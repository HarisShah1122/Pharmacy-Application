...
const PrescriptionDetailForm = () => {
  ...
  const handleDeleteDrug = async (index, drugId) => {
    if (drugId) {
      try {
        const response = await fetch(`http://localhost:8081/prescription-drugs/delete/${drugId}`, {
          method: "DELETE",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (diagnoses.length > 0) {
        await fetch("http://localhost:8081/prescription-diagnosis/add", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
          body: JSON.stringify({ prescription_id: result.data.id, ...diagnoses[0] }),
        });
      }
      if (drugList.length > 0) {
        await fetch("http://localhost:8081/prescription-drugs/add", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          ...
          body: JSON.stringify({
            prescription_id: result.data.id,
            drug_code: drugList[0].ndcDrugCode,
            quantity: drugList[0].dispensedQuantity,
            days_of_supply: drugList[0].daysOfSupply,
            instructions: drugList[0].instructions,
          }),
        });
      }
      ...
      alert(error.message || "An error occurred while saving the prescription details.");
    }
  };
    ...
    <div className="container-fluid">
      <div className="row">
        <main className="col-md-12 p-4">
          <header className="d-flex justify-content-between align-items-center p-3 bg-secondary text-white rounded mb-4">
            <div className="d-flex align-items-center">
              <button className="btn btn-link text-white" onClick={() => navigate("/dashboard/prescription")}>
                <i className="fas fa-arrow-left"></i>
              </button>
              <h2 className="mb-0 ms-3">New Prescription</h2>
            </div>
          </header>
                    ...
                    {formData.drugList.map((drug, index) => (
                      <div key={index} className="row mb-3">
                        ...
                        <div className="col-md-2">
                          ...
                          <textarea className="form-control" name="instructions" value={drug.instructions} onChange={(e) => handleDrugChange(e, index)} required />
                        </div>
                ...
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    {modalType === "diagnosis" ? (
                      <div>
                        <label className="form-label fw-bold">ICD Code</label>
                        <input type="text" className="form-control" name="icdCode" onChange={(e) => handleDiagnosisChange(e, formData.diagnoses.length - 1)} />
                      ...
                      <div>
                        <label className="form-label fw-bold">NDC Drug Code</label>
                        <input type="text" className="form-control" name="ndcDrugCode" onChange={(e) => handleDrugChange(e, formData.drugList.length - 1)} />
                        <label className="form-label fw-bold mt-3">Dispensed Quantity</label>
                        <input type="number" className="form-control" name="dispensedQuantity" onChange={(e) => handleDrugChange(e, formData.drugList.length - 1)} />
                        <label className="form-label fw-bold mt-3">Days of Supply</label>
                        <input type="number" className="form-control" name="daysOfSupply" onChange={(e) => handleDrugChange(e, formData.drugList.length - 1)} />
