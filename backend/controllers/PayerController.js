...
module.exports.controller = function (app) {
  ...
  app.get('/payer', async (req, res) => {
    try {
      console.log('Handling GET /payer request...');
      const { page = 1, limit = 10 } = req.query;
      console.log('Query params:', { page, limit });
      const offset = (page - 1) * limit;
      console.log('Fetching payers with limit:', limit, 'offset:', offset);
      const { count, rows } = await Payer.findAndCountAll({
        limit: parseInt(limit),
        offset,
      });
      console.log('Fetched payers:', { count, rows });
      ...
      console.error('Error in GET /payer:', error);
  ...
  app.get('/payer/:id', async (req, res) => {
    try {
      ...
      console.error('Error in GET /payer/:id:', error);
  ...
  app.put('/payer/:id', updatePayerValidation, async (req, res) => {
    ...
    try {
      ...
      console.error('Error in PUT /payer/:id:', error);
  ...
  app.delete('/payer/:id', async (req, res) => {
    try {
      ...
      console.error('Error in DELETE /payer/:id:', error);
